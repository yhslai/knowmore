...
In each step, if the added code should be unit-tested, add tests in the same step. Avoid 'adding tests at the very last step' pattern.

Each step should be verifiable (either via automatic tests or manual tests). There shouldn't be a step that only does stuff like 'define data structure` without runnable logic.
